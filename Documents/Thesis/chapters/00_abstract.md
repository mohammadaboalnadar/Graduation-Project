...
The trained policy is evaluated on a flat simulation surface. The results demonstrate that the PPO agent successfully learns a stable, coordinated trot-like gait that tracks forward velocity commands with a mean tracking error of **[X.XX] m/s** and maintains stable base roll and pitch orientation within **[Y.YY]** variance. This work establishes a baseline for model-free continuous control on quadrupedal platforms using standard reinforcement learning algorithms without hierarchical structures or reference trajectories.
